...
2026-09-02 更新：第一批工作簿已经提供并确认了 101 条敞顶箱铁路干线测试费率。
其中仅 66 条标准北站—南站 OD 在当前主数据中可唯一准入，按本决策的单位、装卸费、
篷布和时效规则生成 `real_data` 记录；但这些记录尚未默认连接到 full-flow/Web，
更不代表专用线末端、客户交付、顶开门箱或全部铁路业务已经上线。
...
- 敞顶箱篷布费为 250 元/箱，顶开门箱不计该项；
- 首批铁路测试路线的北/南站装卸费均作为独立费用组成：所在地市为韶关市的站点
  为 195 元/箱，其余站点为 136.5 元/箱；堆存、延箱暂不考虑。该规则不表示将
  未确认费用补零；
- 第一批工作簿 `测试路线/敞顶箱-8.28查询` 的数值仅为铁路干线运费，单位为
  元/组且固定 2 箱/组，Adapter 必须转换为元/箱，并不得将上下站装卸费或篷布费
  重复包含在该字段中；
...
- 独立铁路集装箱试算 Demo 的“南站至客户直达拖车/自动”可复用本地优先坐标解析、
  Tencent 地点检索回退和 Tencent 普通驾车道路结果；该能力只用于试算中的末端边，
  不使未维护的铁路干线、站点费或专用线记录成为正式数据。坐标或道路调用失败时
  必须返回 `manual_review`，不得使用人工默认值、零距离或零时效代替；
