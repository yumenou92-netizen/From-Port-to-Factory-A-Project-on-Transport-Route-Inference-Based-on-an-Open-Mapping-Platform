...
- 每完成一个模块的开发、测试和说明更新后，必须提醒进行 Git 提交；如用户明确授权，可代为执行提交并推送到 GitHub。
- `demo_run.py` 作为开发期通用集成测试脚本，可以随着模块演进持续修改，不要求长期保持最简 demo 形态。
- `demo_leader.py` 作为领导展示统一入口；每完成一个模块或小功能，优先把可解释展示接入该入口。
...
- `demo_run.py` 用作开发期通用集成测试入口，可随当前模块重构和扩展。
- `demo_leader.py` 用作面向领导的稳定展示入口，展示内容应强调业务问题、输入、系统判断、输出结论和当前边界。
- 每完成一个模块或小功能，如果能形成可解释输出，应同步接入 `demo_leader.py`；暂时无法可视化展示的模块，需要在阶段汇报中说明原因。
...
- `data_REAL/` 是本地真实业务数据目录，允许在本机开发、数据审计、规则验证和 demo 构建中读取使用。
- 使用 `data_REAL/` 时必须通过 `DATA_DIR` 或本地配置传入，不得在业务代码中硬编码该目录的绝对路径。
- 基于 `data_REAL/` 生成的可提交样例、截图或报告必须先做脱敏或汇总处理。
- 不在未审计数据上直接构图。
- 不把不同费用单位的原始数值直接相加。
- 不把缺失费用或缺失时间默认为 0。
- 不重复创建同一物理南方港口节点。
