...
- 北港至候选南港的散船干线费用和纯航行时间已在当前 Demo 中替换为真实散船运价表和已确认分区时效；南港作业费 Provider 已支持精确费率和已确认地域代理，但正式作业费表及 `operation_fee_region_code` 映射尚未填充，正式节点/客户画像和人工指定南港模式尚未完成；
- 船运时效数据仍存在重大缺口：当前只有北港至南港散船干线纯航行时间；驳船、港口作业、等待、装船、卸船、堆存、短倒和铁路时间均未接入正式数据源。含船运路径的总时间只能作为当前原型内的可比时间，不代表完整门到门 ETA；
- 内河驳船当前只有最小 Demo Provider，可在福建闽江/珠三角生成明确 `demo_placeholder` 航费和纯航行时间；港口能力表和区域映射表已具备 CSV 读取与审计输出，但正式数据尚未填充；正式驳船航费/航时表尚未接入；
...
- AdditionalFee 已加载但尚未完成面向路线段的适配、去重和归属校验；W5 已建立独立南港“码头作业费” Provider 契约，真实费率表未接入或未匹配时仍不计入总费用、不视为 0；只读审计显示当前仍有 2 个站点类 AdditionalFee 节点未注册，按用户确认列入后续集装箱/铁路节点治理，不阻塞当前散粮散船阶段；
...
| `src/data/port_label_rules.py` | 领导提供的部分码头标签 JSON 只读解析；将 `入库` 候选映射为 W5 作业费审计行，不写正式数据 |
...
| `src/routing/port_operation_fee_provider.py` | W5 南港作业费精确/`regional_proxy`/人工复核优先级、CSV 读取和显式 `demo_placeholder` Provider |
...
最近一次全量 pytest 为 `300 passed in 1.78s`。2026-07-24 W3/W5 接口和受影响链路专项测试为 `19 passed in 0.25s`、`20 passed in 0.59s`；随后 W3/W5 样表、full-flow 作业费可选接入、RouteResult 费用组成、Provider 和审计测试为 `32 passed in 0.54s`。本轮补充 `trade_type`、作业费精确单位匹配和 `部分码头标签.json` 审计后，受影响测试为 `44 passed in 0.43s`；最终名称字典、节点注册和标签审计专项测试为 `16 passed in 0.45s`。W5 地域代理、路线标签契约、客户自有码头关系、费用组成、正式图、双目标搜索、full-flow 和审计专项测试为 `123 passed in 0.75s`。只读数据夯实审计已运行，生成 ignored 的 `output/data_foundation_audit.md` 和 `output/data_foundation_audit.json`：当前运价端点未注册为 0，AdditionalFee 节点未注册为 2，散船运价表列数为 18，`秀屿` 已为 `project_scope_confirmed`，正式 W3 港口能力表、区域映射表和 W5 南港码头作业费表记录数当前均为 0。`部分码头标签.json` 刷新审计为 76 条展开候选费率、2 条人工复核规则行、60 条直接节点命中、9 条名称字典唯一后缀匹配、9 条节点未命中，对应 7 个去重名称。真实数据注册表现有 303 个标准节点、0 个坐标冲突；仅余 2 个历史铁路名称别名复核组，与本轮港口标签无关。本轮按用户要求未重复全量 pytest、真实 Tencent API 或完整 real-data smoke。
...
- 新增 `src/routing/bulk_shipping_provider.py`，读取真实散船运价表最新行、保留同一区域多船型列、按订单吨数向上选择船型，并生成真实散船干线费用和已确认纯航行时效；
- `leader_full_flow` 已替换北港至南港干线费用/时效占位，候选若缺少真实散船目的组或纯航行时效分区则不形成干线边；
...
1. 人工确认 `炮台港` 和 `金港` 的标准节点；`汇东/百达/红东` 已明确忽略，不再查询或注册；
2. 确认 `广州番禺灵川饲料有限公司`、`佛山顺德利宝饲料` 的 `入库=0` 是“确实免收”还是“缺失/未维护”；确认前不得生成可用零费用；
