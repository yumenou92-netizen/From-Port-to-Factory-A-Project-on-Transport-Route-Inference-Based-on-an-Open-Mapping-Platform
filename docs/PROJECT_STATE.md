...
本文件是新线程接手项目时的首要工程状态。内容依据当前代码、Git 工作树、最近提交和 2026-07-21 最近一次完整测试结果整理，不以聊天记录或领导汇报代替工程事实。
...
- 162 条已计费候选仍缺少一端或两端标准节点 ID，不能进入正式图；
- 已提供本地去重和候选坐标复核工具；首批 3 个地点经人工确认并在本地补录后，原始缺失地点由 32 个降至 29 个；名称字典只接纳明确全称/简称对；
...
最近一次全量验证为 `259 passed in 1.06s`。此后只修改了名称字典稀疏行兼容处理和本地节点数据，按最小必要验证原则运行相关专项测试：`20 passed in 0.55s`，真实数据链通过。当前真实数据摘要为 492 条运价、314 条已计费候选、152 条可入正式图边、162 条缺节点候选、27 条人工复核记录，去重缺失地点 29 个。Tencent 网络实测仍由用户 PyCharm 环境完成。

## 9. 最近修改内容

提交 `0571921` 完成的阶段 17 工程改动包括：
...
当前分支为 `main...origin/main [ahead 1]`；阶段 17 工程文件已提交到本地，但尚未推送。最近提交：

```text
0571921  2026-07-22  feat(demo): complete leader full-flow rehearsal
...
阶段 17 工程文件已纳入 `0571921`；当前没有其他已跟踪但尚未提交的工程改动。

工作树还存在多份未跟踪的 DOCX、PDF、PPTX、PNG、每日计划/报告。它们是用户办公材料，不应被 `git add .` 一并提交；`.pytest_tmp/` 已在本次交接加入忽略规则。未来提交必须使用文件白名单并先做敏感信息检查。

## 11. 下一步建议

1. 在 PyCharm 用选定北港 A、客户工厂 B 运行 `leader full-flow`，保存脱敏控制台结果并验收展示口径；
2. 只修复预演暴露的阻塞问题，然后对工程文件执行白名单暂存、敏感信息扫描、提交和推送；
3. 展示完成后再进入阶段 18，优先确认北港至南港费用/时效、客户自有码头字段和 AdditionalFee 归属；
4. 制定缺节点治理和 Tencent 低置信结果抽检/缓存/回写协议，再扩大真实业务自动化覆盖。

### 已落实的缺节点治理起步工具

运行 `python -B -m src.demos.node_coordinate_backfill` 可针对当前订单生成去重清单；加 `--query-tencent` 才会在用户本地查询候选坐标。工具只输出 `output/node_coordinate_backfill_review.jsonl`，不修改 `地点经纬度.json`，也不自动认定别名。名称字典只有明确“节点全称 + 节点简称”同一行都存在时，才会为一个已存在坐标节点补充别名；若两边已有不同节点 ID，则保留人工复核。
